...
public abstract class HOp
{
   ...
   * This method routes to "onService(BMPV2Server,HGrid)".
   */
  public void onService(BMPV2Server db, HttpServletRequest req, HttpServletResponse res)
    throws Exception
  {
    ...
    // route to onService(BMPV2Server, HGrid)
    HGrid resGrid;
    try
    {
   ...
   * Service the request and return response.
   */
  public HGrid onService(BMPV2Server db, HGrid req)
    throws Exception
  {
    throw new UnsupportedOperationException(getClass().getName()+".onService(BMPV2Server,HGrid)");
  }
  ...
  private HGridFormat toFormat(HttpServletRequest req)
  {
    ...
    if (format == null) format = HGridFormat.find("application/json", true);
    {
      return format;
    }
  }

  HRef[] gridToIds(BMPV2Server db, HGrid grid)
  {
  ...
  HRef valToId(BMPV2Server db, HVal val)
  {
